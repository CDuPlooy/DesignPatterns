...
Participants :
      Concrete Factory/Creator (Athos/Aramis/Porthos Constructors) -> Overrides the constructor provided by the abstract factory.
      Abstract Factory (Musketeer) -> A constructor which is used to specify a more particular constructor. eg Athos/Aramis/Porthos

      Concrete Product (Athos/Aramis/Porthos Instance) -> Generated by the concrete factory -> Implements the product interface.
      Product (createMuskteer) // sort of. -> Defines the interface for for objects created by the factory.
...
*/
class Musketeer; //Technical note ; In this example the Musketeer base class is acting as an interface and an abstract  factory.

class Athos;
class Aramis;                 //These are forward declarations. They aren't required in this example but they aren't a bad idea either.
class Porthos;

class Musketeer{ //This is an abstract factory.
      public:
...
Musketeer *Musketeer::createMusketeer(short type){  //NB The constructor wrapper itself needs to be below the other child classes. Otherwise we get Incomplete Type errors.
      switch (type) {   //this is the factory method
            case 1:
                  return new Athos;
            case 2:
                  return new Aramis;
